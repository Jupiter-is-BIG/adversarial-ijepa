        ...
        for itr, (udata, masks_enc, masks_pred) in enumerate(unsupervised_loader):
            ...
            def train_step():
                ...
                def forward_discriminator(x, ctx):
                        p = discriminator(x, ctx, masks_enc, masks_pred)
                        return p

                # real_pred_threshold = 0
                ...
                while True:
                    ...
                    real_preds = forward_discriminator(h_target_real.detach(), h_context.detach())
                    fake_preds = forward_discriminator(h_target_fake.detach(), h_context.detach())
                    print(real_preds.shape)
                    print(torch.ones_like(real_preds).shape)
                    ...
                    # real_pred_threshold = mean_real_preds
                    print("Mean of real predictions: %.3f | Mean of fake predictions: %.3f" % (mean_real_preds, mean_fake_preds))

                    # if mean_real_preds - mean_fake_preds >= 0.8:
                    #     break
                ...
                while True:
                    ...
                    mean_fake_preds = torch.mean(fake_preds).item()
                    print("Mean of fake predictions pre-train %.3f" % (mean_fake_preds))

                    reconstruction_loss = F.mse_loss(h_target_fake, h_target_real)
                    g_loss = criterion(fake_preds, torch.ones_like(fake_preds))

                    print("Reconstruction Loss %.3f" % (reconstruction_loss.item()))
                    # if mean_fake_preds >= real_pred_threshold:
                    #     break

                    combined_loss =  0.32 * reconstruction_loss + g_loss
